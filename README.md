...
身份。文件在快照期间仍发生变化时会留待下次运行。空 Markdown 会进入现有 Ingest 校验流程并成为最终
失败记录，而不会每天无限延后。每个文件在一次定时同步中只会创建一个 Ingest 任务；任务失败时会保留
失败审计，但删除 `raw/uploads/` 中该任务的上传源文件，不再每日重试。同步命令只经
`http://127.0.0.1:8081` 调用现有 Ingest API，成功任务仍由后端自动排队
Quartz 发布。
