...
> 状态（2026-08-10）：已按当前代码实现。后续运维以 `README.md` 的“每日增量 Markdown 入库”、
> `app/services/scheduled_ingest_service.py` 和 systemd 示例为准；本文保留设计过程，但下列
> 已确认边界已同步到现行实现。
...
- 每个文件在一次同步中只提交一个 Ingest 任务；失败后立即标记为最终失败并记录告警，以后不自动重试。
...
只负责扫描、去重、单次提交与日志。该命令只访问本机 loopback API，不增加公网入口。
...
attempt_count TINYINT UNSIGNED NOT NULL     -- 已记录的提交尝试次数；当前正常流程为 1
...
- `failed`：本次提交、轮询或 Ingest 失败，以后跳过并保留错误审计。
...
7. 上传、轮询或 Ingest 失败时将清单记为 `failed`，保留简短错误，记录
   `WARNING`/`ERROR` 日志并继续下一个文件；不自动重传，以免无法确认的请求造成重复 Wiki 写入。
...
- 每次定时提交使用带随机 UUID 的暂存文件名，避免不同源目录中的同名文件互相冲突。
...
- 与现有 10 MiB 上传限制保持一致；超过限制的 `.md` 视为最终失败，不自动重试。
...
| `app/services/scheduled_ingest_service.py` | 扫描、去重、单次调用/轮询 loopback API 和日志 |
...
| `tests/test_scheduled_ingest_service.py` | 扫描、首次全量、增量、符号链接、重复、失败不重试、断点恢复 |
...
- 每个文件在一次同步中最多提交一次，最终失败有可追踪日志，不会每天重复消耗模型资源。
